...
import './material-types-edit.scss';
import TypesTable from '../../../../components/types-table/types-table/types-table';
import useFetch from '../../../../hooks/use-fetch';
import Modal from '../../../../components/modal/modal';
import MaterialTypesEditTable from "../material-types-edit-table/material-types-edit-table";

const MaterialTypesEdit = ({ onClose, itemId, reloadTable }) => {
  ...
  const [{response, isLoading, error}, doFetchItem] = useFetch(`/directories/material_types/${itemId}`);
  const [{response: itemUpdateResponse, isLoading: itemUpdateIsLoading, error: itemUpdateError}, doFetchItemUpdate] = useFetch(`/directories/material_types/${itemId}`);
  const [{response: itemSaveResponse, isLoading: itemSaveIsLoading, error: itemSaveError}, doFetchItemSave] = useFetch(`/directories/material_types`);
  ...
  const tableStructure = {
    tableClass: 'types-table-add',
    colsList: [
      {
        name: 'label',
        colClass: 'characteristic',
  ...
  return (
    <Modal
      ...
      isWide={true}
    >
      <div className="material-types-edit">
        <div className="material-types-edit-name">
          <input
            ref={nameInput}
            autoFocus={nameEmpty}
            type="text"
            placeholder="Название типа оборудования"
            className={`material-types-edit-name__input${nameEmpty ? ' error' : ''}`}
            onChange={(evt) => setName(evt.target.value)}
            value={name}
          />
          {nameEmpty ? (
            <span className="material-types-edit__error">Введите название типа оборудования</span>
          ) : ''}
        </div>
        <MaterialTypesEditTable
          id={itemId}
        ...
        />
        <button
          className="material-types-edit__btn btn"
          type="button"
          onClick={onAddRow}
        >Добавить</button>
      </div>
...
export default MaterialTypesEdit;
