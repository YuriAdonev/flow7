...
import './material-types-table.scss';
...
const MaterialTypesTable = ({ error, tableData, isLoading, onItemDelete, onItemEdit, sortedBy, sortedDown, onSortClick }) => {
  const tableActions = {
    title: 'Действия',
    itemsList: [
      {
  ...
  const items = tableData.map(tableItem => {
    return (
      ...
      >
        <td
          className="material-types-table__name table__col table__col--first"
  ...
  return (
    <table className={`material-types-table table`}>
      <thead>
      <tr
        className="table__row table__row--head"
      >
        <th className="material-types-table__name table__col table__col--first table__col--head">
          <SortItem
            label="Название"
            name="name"
            sortedBy={sortedBy}
