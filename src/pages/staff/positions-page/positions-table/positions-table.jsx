...
const PositionsTable = ({ error, tableData, isLoading, onItemDelete, onItemEdit, sortedBy, sortedDown, onSortClick }) => {
  ...
  const items = tableData.map(tableItem => {
    return (
      ...
      >
        <td
          className="positions-table__name table__col table__col--first"
  ...
  return (
    <table className={`positions-table table`}>
      ...
      >
        <th className="positions-table__name table__col table__col--first table__col--head">
          <SortItem
            label="Должность"
            name="name"
            sortedBy={sortedBy}
